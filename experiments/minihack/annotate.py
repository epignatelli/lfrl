...
from calf.annotate import annotate_subgoals
...
def main():
    ...
    # static
    annotate = partial(annotate_subgoals, seq_len=seq_len)
    ...
    for demo_batch in load_pickle_minibatches(source_path, batch_size):
        ...
        annotations = annotate(demo_batch, max_new_tokens=max_new_tokens, url=url)
        for ann in annotations:
            table = wandb.Table(
                columns=["Prompts", "Response", "Parsed"],
                data=[
                    [
                        str(ann.prompt),
                        str(ann.response),
            ...
            wandb.log({"Annotations": table}, commit=False)
        print(
            f"Saving annotations {i * batch_size}-{(i + 1) * batch_size} to {dest_path}"
        )
        wandb.log({"calf/annotation_size": annotation_size})
