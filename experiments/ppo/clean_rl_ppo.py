...
from calm.environment import ShaperWrapper, UndictWrapper
...
class Args:
    ...
    observation_key: str = "chars"
    """The main observation key for training"""
    env_name: str = "MiniHack-KeyRoom-S5-v0"
...
if __name__ == "__main__":
    ...
    envs = gymnasium.vector.make(
        ...
        wrappers=[
            ...
            # lambda x: UndictWrapper(x, key=args.observation_key),
        ],
    )
    envs = UndictWrapper(envs, key=args.observation_key)  # to the vector env
    # assert isinstance(
    ...
    for iteration in range(1, args.num_iterations + 1):
            ...
            if "final_info" in infos:
                for info in infos["final_info"]:
                    if info and "episode" in info:
                        ...
                        wandb.log(
                            {"charts/episodic_return": info["episode"]["r"]}, step=global_step
                        )
                        wandb.log(
        ...
        fps = (args.num_steps * args.num_envs) / (time.time() - start_time)
        wandb.log(
        ...
        wandb.log({"charts/frames": frames}, step=global_step)
        wandb.log({"charts/fps":fps}, step=global_step)
